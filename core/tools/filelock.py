...
## \package pts.core.tools.filelock Contains the FileLock class.
...
try: TimeoutError
except NameError: TimeoutError = RuntimeError
    ...
    def acquire(self, timeout=None, poll_intervall=0.05):
        ...
        try:
            ...
            while True:
                ...
                if self.is_locked: break
                elif timeout >= 0 and time.time() - start_time > timeout: raise Timeout(self._lock_file)
                else: time.wait(poll_intervall)

        except:
            # Something did go wrong, so decrement the counter.
            with self._thread_lock: self._lock_counter = max(0, self._lock_counter - 1)
            raise
...
if msvcrt: FileLock = WindowsFileLock
elif fcntl: FileLock = UnixFileLock
else:
