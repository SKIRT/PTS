...
## This function is Sam's implementation of a functionality similar to 'createsimulations', without having
#  to change the behaviour of that function
def load_simulations(search_path, recursive=True):
    ...
    for path, name in fs.files_in_path(search_path, extension="ski", recursive=recursive, returns=["path", "name"]):
    ...
    for path, name in fs.files_in_path(search_path, extension="xml", endswith="_parameters", recursive=recursive, returns=["path", "name"]):
    ...
    for path, name in fs.files_in_path(search_path, extension="txt", contains="log", recursive=recursive, returns=["path", "name"]):
